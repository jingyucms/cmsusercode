...
if __name__ == '__main__':
    ...
    for prefix in prefixs: 
     ...
     for i in range(len(samples)):
      ...
      # PDF1 correction
      filename1nu1="../fastnlo/fnl3622g_mstw2008-nlo_aspdf.root"
      print filename1nu1
      nlofile1 = TFile.Open(filename1nu1)

      # PDF2 correction
      filename1nu2="../fastnlo/fnl3622g_nnpdf21-nlo_aspdf.root"
      print filename1nu2
	...
	for k in range(0,200):
	  ...
	  for k in range(0,200):
        ...
        for k in mass_bins_nlo2[j]:
         ...
         histname+=str(k)+"00"
         print histname
         hnlo = TH1F(nlofile1.Get(histname))
         hnlo.Scale(float(mass_bins_nlo[k+1]-mass_bins_nlo[k]))
         hnlo=rebin(hnlo,len(chi_binnings[j])-1,chi_binnings[j])
	 if nlopdf1:
	    nlopdf1.Add(hnlo)
	    ...
	    nlopdf1=hnlo
        for b in range(nlopdf1.GetXaxis().GetNbins()):
            nlopdf1.SetBinContent(b+1,nlopdf1.GetBinContent(b+1)*nlopdf1.GetBinWidth(b+1))
	for b in range(nlopdf1.GetXaxis().GetNbins()):
	    low_bin=ewk.FindBin(nlopdf1.GetXaxis().GetBinLowEdge(b+1))
	    up_bin=ewk.FindBin(nlopdf1.GetXaxis().GetBinUpEdge(b+1))
	    correction=ewk.Integral(low_bin,up_bin-1)/(up_bin-low_bin)
	    nlopdf1.SetBinContent(b+1,nlopdf1.GetBinContent(b+1)*correction)
        nlopdf1.Scale(dataevents[j]/nlopdf1.Integral())
        for b in range(nlopdf1.GetXaxis().GetNbins()):
            nlopdf1.SetBinError(b+1,0)
	 ...
	 if nlopdf2:
	    ...
	    nlopdf2=hnlo
        for b in range(nlopdf2.GetXaxis().GetNbins()):
            nlopdf2.SetBinContent(b+1,nlopdf2.GetBinContent(b+1)*nlopdf2.GetBinWidth(b+1))
	for b in range(nlopdf2.GetXaxis().GetNbins()):
	    low_bin=ewk.FindBin(nlopdf2.GetXaxis().GetBinLowEdge(b+1))
	    up_bin=ewk.FindBin(nlopdf2.GetXaxis().GetBinUpEdge(b+1))
	    correction=ewk.Integral(low_bin,up_bin-1)/(up_bin-low_bin)
	    nlopdf2.SetBinContent(b+1,nlopdf2.GetBinContent(b+1)*correction)
        nlopdf2.Scale(dataevents[j]/nlopdf2.Integral())
        for b in range(nlopdf2.GetXaxis().GetNbins()):
            nlopdf2.SetBinError(b+1,0)
	...
	#ci=alt.Clone("fake_signal")
        ...
        nlopdf1.Divide(nlopdf1,alt)
        nlopdf2.Divide(nlopdf2,alt)
        alt.Divide(alt,alt)
	...
	alt.SetTitle("")
        alt.Draw("he")
	alt.GetYaxis().SetRangeUser(0.98,1.03)
	...
	jesdown.SetLineStyle(3)
        #jesdown.Draw("hesame")
	plots+=[pdfup]
	pdfup.SetLineColor(1)
	pdfup.SetLineStyle(4)
        pdfup.Draw("hesame")
        legend1.AddEntry(pdfup,"CT10 68% band","l")
	plots+=[pdfdown]
	pdfdown.SetLineColor(1)
	pdfdown.SetLineStyle(4)
        pdfdown.Draw("hesame")
        legend1.AddEntry(nlopdf1,"MSTW2008","l")
	plots+=[nlopdf1]
	nlopdf1.SetLineColor(3)
	nlopdf1.SetLineStyle(2)
        nlopdf1.Draw("hesame")
        legend1.AddEntry(nlopdf2,"NNPDF21","l")
	plots+=[nlopdf2]
	nlopdf2.SetLineColor(4)
	nlopdf2.SetLineStyle(3)
        nlopdf2.Draw("hesame")
	plots+=[scaleup]
	...
	plots+=[l5]
      ...
      canvas.SaveAs(prefix + "_"+samples[i][0] + '_pdf.pdf')
      canvas.SaveAs(prefix + "_"+samples[i][0] + '_pdf.eps')
