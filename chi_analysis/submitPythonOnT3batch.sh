...
gfal-mkdir $USER_SRM_HOME/$HN_NAME/$SEUSERSUBDIR/$6
if test x"$SEUSERSUBDIR" = x; then
   SERESULTDIR=$USER_SRM_HOME/$HN_NAME/$6
   SERESULTDIRXRD=$USER_XRD_HOME/$HN_NAME/$6
else
   SERESULTDIR=$USER_SRM_HOME/$HN_NAME/$SEUSERSUBDIR/$6
   SERESULTDIRXRD=$USER_XRD_HOME/$HN_NAME/$SEUSERSUBDIR/$6
...
echo "python $CMSSW_CONFIG_FILE $JOB_ID>> myout.txt 2>> myerr.txt"
python $CMSSW_CONFIG_FILE $JOB_ID>> myout.txt 2>> myerr.txt
...
if test 0"$DBG" -gt 0; then
    ...
    ls -Rl
    echo "########################################################"
    echo "YOUR OUTPUT LOGS WILL BE MOVED TO $RESULTDIR"
    echo "YOUR OUTPUT FILES WILL BE MOVED TO $SERESULTDIR"
    echo "########################################################"
fi
...
if test x"$SEOUTFILES" != x; then
   ...
   for n in $SEOUTFILES; do
       if test ! -e $WORKDIR/${n}.root; then
          echo "WARNING: Cannot find output file $WORKDIR/${n}.root. Ignoring it" >&2
       else
          #lcg-cp $srmdebug -b -D srmv2 file:$WORKDIR/$n $SERESULTDIR/$n
          xrdcp -d 1 $WORKDIR/${n}_tree.root $SERESULTDIRXRD/${n}_tree.root -f
          cp -a $WORKDIR/${n}.root $RESULTDIR/${n}.root -f
          #xrdcp -d 1 $WORKDIR/${n}.root $SERESULTDIRXRD/${n}.root -f
	  #cp $WORKDIR/$n /afs/cern.ch/work/z/zhangj/public/forChi/$n
          if test $? -ne 0; then
