...
int extractGoodnessStats(TString prefix){

  TString fileName="higgsCombine"+prefix+"goodnessfittoys.GoodnessOfFit.mH120.123456.root";
  TFile *fq=new TFile(fileName,"READ");
  TTree *t=(TTree*)fq->Get("limit");

  double q,m,w;
  int type;
  t->SetBranchAddress("limit",&q);
  ...
  medianPS=v_PS.at(int(ntoysPS*0.5));
  cout<<"Expected median = "<<medianPS<<endl;
  medianPS=v_PS.at(int(ntoysPS*0.159));
  cout<<"Expected - 1sigma = "<<medianPS<<endl;
  medianPS=v_PS.at(int(ntoysPS*0.841));
  cout<<"Expected + 1sigma = "<<medianPS<<endl;
  medianPS=v_PS.at(int(ntoysPS*0.023));
  cout<<"Expected - 2sigma = "<<medianPS<<endl;
