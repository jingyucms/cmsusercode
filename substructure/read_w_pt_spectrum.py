...
for boson,name in [("w","enj"),("z","eej")]:
  ...
  gkNNLO=TGraph(len(pointsX),pointsX,pointsY)
  gkNNLO.SetName("kNNLO")

  pointsX=array.array("f",pt_bins)
  pointsY=array.array("f",d1kNLO)
  gd1kNLO=TGraph(len(pointsX),pointsX,pointsY)
  gd1kNLO.SetName("d1kNLO")

  pointsX=array.array("f",pt_bins)
  pointsY=array.array("f",d2kNLO)
  gd2kNLO=TGraph(len(pointsX),pointsX,pointsY)
  gd2kNLO.SetName("d2kNLO")

  pointsX=array.array("f",pt_bins)
  pointsY=array.array("f",d1kappaEW)
  gd1kappaEW=TGraph(len(pointsX),pointsX,pointsY)
  gd1kappaEW.SetName("d1kappaEW")

  pointsX=array.array("f",pt_bins)
  pointsY=array.array("f",d2kappaEW)
  gd2kappaEW=TGraph(len(pointsX),pointsX,pointsY)
  gd2kappaEW.SetName("d2kappaEW")

  pointsX=array.array("f",pt_bins)
  pointsY=array.array("f",dPDF)
  gdPDF=TGraph(len(pointsX),pointsX,pointsY)
  gdPDF.SetName("dPDF")

  out=TFile(boson+"_pt_spectrum.root","RECREATE")
  gkNNLO.Write()
  gd1kNLO.Write()
  gd2kNLO.Write()
  gd1kappaEW.Write()
