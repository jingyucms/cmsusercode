...
prefix = "plots/tauisolation_offlineVertex_offpix"
wait=True
...
def makeEfficiency(passing, all):
    ...
    gragh.GetYaxis().SetRangeUser(0.5,1)
    return gragh
...
if __name__ == '__main__':
    ...
    vertex_n = TH1F('tau pT 20 vertex n',';N(vertices);N',15,0,29)
    vertex_n.Sumw2()

    filter_vertex_n = TH1F('tau pT 20 vertex n eff',';N(vertices);Trigger efficiency',15,0,29)
    filter_vertex_n.Sumw2()

    filter2_vertex_n = TH1F('tau pT 20 vertex n eff2',';N(vertices);Trigger efficiency',15,0,29)
    filter2_vertex_n.Sumw2()
    filter2_vertex_n.SetLineColor(2)

    filter3_vertex_n = TH1F('tau pT 20 vertex n eff3',';N(vertices);Trigger efficiency',15,0,29)
    filter3_vertex_n.Sumw2()
    filter3_vertex_n.SetLineColor(4)

    filter4_vertex_n = TH1F('tau pT 20 vertex n eff4',';N(vertices);Trigger efficiency',15,0,29)
    filter4_vertex_n.Sumw2()
    filter4_vertex_n.SetLineColor(3)

    veff_vertex_n = TH1F('tau pT 20 vertex 1 eff',';N(vertices);Primary vertex matched',15,0,29)
    veff_vertex_n.Sumw2()

    vreco_vertex_n = TH1F('tau pT 20 vertex 1 reco',';N(vertices);Primary vertex reconstructed',15,0,29)
    vreco_vertex_n.Sumw2()

    hltvertex_n = TH1F('tau pT 20 hltvertex n',';N(HLT-vertices);N',10,0,19)
    hltvertex_n.Sumw2()

    filter_hltvertex_n = TH1F('tau pT 20 hltvertex n eff',';N(vertices);Trigger efficiency',10,0,19)
    ...
    #isolation4filter_label="hltPFTauMediumIso20TrackMediumIso"
	...
	#matched_tau_20=electronfilter.electronSize()>0
	#matched_tau_20=muonfilter.muonSize()>0
    ...
    B_vertex_n = TH1F('tau pT 20 vertex n B',';N(vertices);N',15,0,29)
    B_vertex_n.Sumw2()

    filterB_vertex_n = TH1F('tau pT 20 vertex n Beff',';N(vertices);Trigger efficiency',15,0,29)
    filterB_vertex_n.Sumw2()

    filterB2_vertex_n = TH1F('tau pT 20 vertex n Beff2',';N(vertices);Trigger efficiency',15,0,29)
    filterB2_vertex_n.Sumw2()
    filterB2_vertex_n.SetLineColor(2)

    filterB3_vertex_n = TH1F('tau pT 20 vertex n Beff3',';N(vertices);Trigger efficiency',15,0,29)
    filterB3_vertex_n.Sumw2()
    filterB3_vertex_n.SetLineColor(4)

    filterB4_vertex_n = TH1F('tau pT 20 vertex n Beff4',';N(vertices);Trigger efficiency',15,0,29)
    filterB4_vertex_n.Sumw2()
    filterB4_vertex_n.SetLineColor(3)

    events2=Events("/tmp/hinzmann/trigger_study_offlineVertex_pix.root")

    event_count=0
    for event in events2:
        event_count+=1
	#print "Event:",event_count
        events2.getByLabel(tau_label,tau_handle)
        taus=tau_handle.product()
        events2.getByLabel(hlttau_label,hlttau_handle)
        hlttaus=hlttau_handle.product()
        events2.getByLabel(l1filter_label,l1filter_handle)
        l1filter=l1filter_handle.product()
        events2.getByLabel(elefilter_label,elefilter_handle)
        elefilter=elefilter_handle.product()
        events2.getByLabel(isolationfilter_label,isolationfilter_handle)
        isolationfilter=isolationfilter_handle.product()
        events2.getByLabel(isolation2filter_label,isolation2filter_handle)
        isolation2filter=isolation2filter_handle.product()
        events2.getByLabel(isolation3filter_label,isolation3filter_handle)
        isolation3filter=isolation3filter_handle.product()
        events2.getByLabel(isolation4filter_label,isolation4filter_handle)
        isolation4filter=isolation4filter_handle.product()
	try:
            #events2.getByLabel(electronfilter_label,electronfilter_handle)
            #electronfilter=electronfilter_handle.product()
            #events2.getByLabel(muonfilter_label,muonfilter_handle)
            #muonfilter=muonfilter_handle.product()
            events2.getByLabel(hltvertex_label,"","TEST",hltvertex_handle)
            hltvertices=hltvertex_handle.product()
	except: continue
        events2.getByLabel(vertex_label,"","RECO",vertex_handle)
        vertices=vertex_handle.product()
	...
	#matched_tau_20=muonfilter.muonSize()>0
		...
		first_hltvertex=False
	if matched_tau_20:
            B_vertex_n.Fill(num_vertices)
	if matched_tau_20_filter:
            filterB_vertex_n.Fill(num_vertices)
	if matched_tau_20_filter2:
            filterB2_vertex_n.Fill(num_vertices)
	if matched_tau_20_filter3:
            filterB3_vertex_n.Fill(num_vertices)
	if matched_tau_20_filter4:
            filterB4_vertex_n.Fill(num_vertices)
    ...
    filterB3_vertex_n.Divide(filterB3_vertex_n,filter3_vertex_n,1,1)
    filterB3_vertex_n.Draw("le")
    filterB3_vertex_n.GetYaxis().SetRangeUser(0.5,1.1)
    #graphv3=makeEfficiency(filterB3_vertex_n,filter3_vertex_n)
    #graphv3.Draw("apz")
    fit3=TF1('fit graphv3','[0]+[1]*x',5,31)
    fit3.SetLineColor(4)
    #graphv3.Fit(fit3,"R0")
    filterB4_vertex_n.Divide(filterB4_vertex_n,filter4_vertex_n,1,1)
    filterB4_vertex_n.Draw("lesame")
    filterB4_vertex_n.GetYaxis().SetRangeUser(0.5,1.1)
    #graphv4=makeEfficiency(filterB4_vertex_n,filter4_vertex_n)
    #graphv4.Draw("pzsame")
    fit4=TF1('fit graphv4','[0]+[1]*x',5,31)
    fit4.SetLineColor(3)
    #graphv4.Fit(fit4,"R0")
    filterB2_vertex_n.Divide(filterB2_vertex_n,filter2_vertex_n,1,1)
    filterB2_vertex_n.Draw("lesame")
    filterB2_vertex_n.GetYaxis().SetRangeUser(0.5,1.1)
    #graphv2=makeEfficiency(filterB2_vertex_n,filter2_vertex_n)
    #graphv2.Draw("pzsame")
    fit2=TF1('fit graphv2','[0]+[1]*x',5,31)
    fit2.SetLineColor(2)
    #graphv2.Fit(fit2,"R0")
    filterB_vertex_n.Divide(filterB_vertex_n,filter_vertex_n,1,1)
    filterB_vertex_n.Draw("lesame")
    filterB_vertex_n.GetYaxis().SetRangeUser(0.5,1.1)
    #graphv=makeEfficiency(filterB_vertex_n,filter_vertex_n)
    #graphv.Draw("pzsame")
    ...
    #fit2.Draw('lsame')
    #fit.Draw('lsame')
    #print fit3.Eval(5),fit3.Eval(20)
    #print fit4.Eval(5),fit4.Eval(20)
    #print fit2.Eval(5),fit2.Eval(20)
    #print fit.Eval(5),fit.Eval(20)
    ...
    canvas.SaveAs(prefix + '_comp.root')
    canvas.SaveAs(prefix + '_comp.pdf')
    canvas.SaveAs(prefix + '_comp.eps')
    if wait:
        os.system("ghostview "+prefix + '_comp.eps')
